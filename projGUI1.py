...
def set_cell_value(event):
    ...
    def saveedit():
        treeview.set(item, column=column, value=entryedit.get(0.0, "end").replace("\n", ""))
        entryedit.destroy()
        okb.destroy()
        uokb.destroy()
    ...
    uokb = ttk.Button(root, text='cancel', width=4, command=unsaveedit)
...
def videocompose():
    ...
    if project_dir == "":
        ...
        if name:  # 如果输入不为空
            ...
            scriptlist = []  # 定义一个空列表 scriptlist，用于存储文本片段
            audioflist = []  # 定义一个空列表 audioflist，用于存储音频文件
            imgflist = []  # 定义一个空列表 imgflist，用于存储图片文件
            for item in treeview.get_children(''):  # 遍历表格内容
                scriptlist.append(treeview.item(item, 'values')[0])  # 获取每一行的第一列数据，添加到 scriptlist 列表中
                audioflist.append(treeview.item(item, 'values')[3])  # 获取每一行的第四列数据，添加到 audioflist 列表中
                imgflist.append(treeview.item(item, 'values')[4])  # 获取每一行的第五列数据，添加到 imgflist 列表中
            # 调用 composevideo_2_compose_TAI_listver 函数，根据文本片段、音频文件、图片文件和项目文件夹地址生成视频文件列表
            save_files_list = composevideo_2_compose_TAI_listver(scriptlist, audioflist, imgflist, project_dir, name)
            if save_files_list:
                # 填充 tvsmtable 的 f 列
                for i in range(len(save_files_list)):
                    treeview.set(treeview.get_children('')[i], column=5, value=save_files_list[i])  # 设置每一行的第六列数据
                print("视频合成成功")
            else:
                print("视频合成失败")
...
# 设置按键位置和大小
select_dir_btn.place(x=10, y=10, width=150, height=30)

# 创建初始化项目文件夹按键
init_dir_btn = ttk.Button(root, text="初始化项目文件夹", command=init_project_dir)
# 设置按键位置和大小
init_dir_btn.place(x=170, y=10, width=150, height=30)
...
# 设置表格位置和大小
treeview.place(x=10, y=50, width=1180, height=300)

# 创建导出 tree 的内容按键，任何时候都可以点击，保存 tvsmtable 的内容，保存在 scriptlist 文件夹中，保存为 json 格式，需要用户对文件进行手动命名。调用函数名 tvsmtable_save()
export_btn = ttk.Button(root, text="导出 table 的内容", command=tvsmtable_save)
# 设置按键位置和大小
export_btn.place(x=10, y=360, width=150, height=30)

# 创建导入信息进 tvsmtable 按键。选择文件，将文件信息导入进 tvsmtable。调用函数名 tvsmtable_load()
import_btn = ttk.Button(root, text="导入信息进 table", command=tvsmtable_load)
# 设置按键位置和大小
import_btn.place(x=170, y=360, width=150, height=30)
...
# 设置按键位置和大小
gen_text_btn.place(x=330, y=360, width=150, height=30)

# 创建文案显示的一个文本框
text_box = Text(root)
# 设置文本框位置和大小
text_box.place(x=10, y=400, width=1180, height=100)

# 创建导入文案按键。选择一个文案文件夹，可以导入
import_text_btn = ttk.Button(root, text="导入文本", command=lambda: text_box.insert(1.0, askopenfilename(initialdir=os.path.join(project_dir, "scripts"))))
# 设置按键位置和大小
import_text_btn.place(x=490, y=360, width=150, height=30)
...
# 设置按键位置和大小
split_text_btn.place(x=650, y=360, width=150, height=30)
...
# 设置按键位置和大小
gen_audio_btn.place(x=810, y=360, width=150, height=30)
...
# 设置按键位置和大小
extract_keywords_btn.place(x=970, y=360, width=150, height=30)
...
# 设置按键位置和大小
get_img_btn.place(x=10, y=510, width=150, height=30)
...
# 设置按键位置和大小
compose_video_btn.place(x=170, y=510, width=150, height=30)
