...
def split_script(script):
    ...
    # 定义一个自定义的提示，用于拆分文案
    myprompt = "对我给出的资料，我需要你把它拆分成意义独立的句子块。即单独看这一段句子块，意义也是独立完整的。可以作为一个部件拼到其他地方,每一个句子块不应太长，符合的情况下尽可能拆分。每两个句子块之间用$分割（务必记得用$分割)。不要使用1.2.3.等序号，不要加任何多余的分隔符\n我给出的资料：" + script + "\n\n句子："
    ...
    try:
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "你是一位十分严谨的对语言敏感的文字工作者.（务必记得按要求用$分割)"},
